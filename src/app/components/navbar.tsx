...
import { Menu, X, LogOut, User as UserIcon, LayoutDashboard, PlusCircle, Briefcase, Search, ShieldCheck } from "lucide-react";
...
export function Navbar() {
  ...
  const NavLinks = () => {
    ...
    if (user.role === "ADMIN") {
      return (
        <>
          <Link href="/admin/dashboard" className="hover:text-secondary-foreground flex items-center gap-1">
            <LayoutDashboard size={16} /> Dashboard
          </Link>
          <Link href="/admin/verifications" className="hover:text-secondary-foreground flex items-center gap-1">
    ...
    <nav className="bg-primary text-primary-foreground sticky top-0 z-50 shadow-md">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16 items-center">
          ...
          <div className="hidden md:flex gap-8 items-center font-medium text-sm">
            ...
            {!isLoading && !user && (
              <div className="flex items-center gap-4 ml-4">
                <Link href="/auth/login" className="hover:text-secondary-foreground">
                  Login
                </Link>
                <Button
                  asChild
            ...
            {!isLoading && user && (
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" className="relative h-10 w-10 rounded-full bg-secondary/20 hover:bg-secondary/30 ring-2 ring-secondary/40 ml-2">
                ...
                <DropdownMenuContent className="w-56" align="end" forceMount>
                  <DropdownMenuLabel className="font-normal">
                    <div className="flex flex-col space-y-1">
                      ...
                      <Badge variant="outline" className="mt-1 w-fit text-[10px] py-0">{user.role}</Badge>
                    </div>
                  </DropdownMenuLabel>
                  <DropdownMenuSeparator />
          ...
          {/* Mobile Menu Button */}
          <button className="md:hidden p-2 rounded-md hover:bg-primary-foreground/10" onClick={() => setIsOpen(!isOpen)}>
        ...
        {isOpen && (
          <div className="md:hidden pb-4 pt-2 space-y-3 border-t border-white/10 mt-2">
            ...
            {user ? (
               <>
                 <Button onClick={handleLogout} className="w-full justify-start text-red-100 hover:bg-red-500/10" variant="ghost">
